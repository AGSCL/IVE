...
paste(
    posterior_polca_05 %>% 
        ...
        dplyr::mutate_at(c("percent", "valid_percent"),~round(.,3)*100) %>% 
        dplyr::mutate(format=paste0(`n`," (",valid_percent,"%)")) %>% 
        dplyr::select(format)%>%  unlist(),
    collapse=", ") 
...
LCA_best_model_mod$posterior %>% 
  ...
  dplyr::mutate(final_07=dplyr::case_when(`X1`==1~1,`X2`==1~2, `X3`==1~3,`X4`==1~4, `X5`==1~5))%>% 
  rowwise() %>%
  mutate(count_ones = sum(c_across(starts_with("X")) == 1)) %>%
  ungroup() %>% 
  janitor::tabyl(final_07, count_ones)
...
bd_reg<-
bind_rows(
broom::tidy(glm(outcome~factor(final_07), data=cbind.data.frame(mydata_preds3,final_07=posterior_polca_07adj$final_07,final_05=posterior_polca_05adj$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0))),
            exponentiate=T, conf.int = T
),
broom::tidy(glm(outcome~factor(final_05), data=cbind.data.frame(mydata_preds3,final_07=posterior_polca_07adj$final_07,final_05=posterior_polca_05adj$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0))),
            exponentiate=T, conf.int = T
),
broom::tidy(glm(outcome~factor(final_07), data=cbind.data.frame(mydata_preds3,final_07=posterior_polca_07$final_07,final_05=posterior_polca_05$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0))),
            exponentiate=T, conf.int = T
),
broom::tidy(glm(outcome~factor(final_05), data=cbind.data.frame(mydata_preds3,final_07=posterior_polca_07$final_07,final_05=posterior_polca_05$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0))),
            exponentiate=T, conf.int = T
)
) %>% 
  ...
  add_column(reg=c(rep(">.7 probs adj",5),rep(">.5 probs adj",5),rep(">.7 probs",5),rep(">.5 probs",5))) %>% 
  dplyr::select(reg, everything())
