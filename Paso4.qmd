...
df_best_model_glca_w_distal_outcome<-
  data.frame(best_model_glca_w_distal_outcome$coefficient) %>% 
  rownames_to_column("term") %>% janitor::clean_names() %>%
...
df_best_model_glca_w_distal_outcome2<-
  ...
  dplyr::mutate(
    odds_ratio_int = exp(`(Intercept)_coefficient`),
    odds_ratio_comp = exp(`outcome1_coefficient`),
    probability_int = odds_ratio_int / (1 + odds_ratio_int),
    probability_comp = odds_ratio_comp / (1 + odds_ratio_comp),
    lower_log_or_int = `(Intercept)_coefficient` - 1.96 * `(Intercept)_std_error`,
    upper_log_or_int = `(Intercept)_coefficient` + 1.96 * `outcome1_coefficient`,
    ...
    lower_int = exp(lower_log_or_int) / (1 + exp(lower_log_or_int)),
    upper_int = exp(upper_log_or_int) / (1 + exp(upper_log_or_int)),
    lower_comp = exp(lower_log_or_comp) / (1 + exp(lower_log_or_comp)),
    upper_comp = exp(upper_log_or_comp) / (1 + exp(upper_log_or_comp))) %>%
  dplyr::select(prefix,
                odds_ratio_int, odds_ratio_comp,
                probability_int, lower_int, upper_int,
                probability_comp, lower_comp, upper_comp)

df_best_model_glca_w_distal_outcome2 %>% 
  knitr::kable("markdown", caption="Table of probabilities of LCA w/distal outcome")
```
