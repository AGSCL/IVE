...
zp1b <- ggplot(lcmodel %>% 
                 ...
                 L2= dplyr::case_when(L2=="CAUSAL"~ "Causal",
                                    ...
                                    L2=="PAIS_ORIGEN_REC"~ "País de origen")) %>% dplyr::mutate(lab2=glue::glue("{stringr::str_replace(CATEGORIA,' ','\n')}\n({scales::percent(value)})")),aes(x = L2, y = value, fill = Var2, label=lab2))
...
ggsave("zp1.png", zp1b+ggrepel::geom_label_repel(#aes(#y=half, label=lab),
...
bd_reg2<-
bind_rows(
broom::tidy(glm(outcome~factor(final_07), data=cbind.data.frame(mydata_preds3,final_07=posterior_glca_07_final$final_07,final_05=posterior_glca_05_final$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0)) %>% dplyr::filter(!is.na(final_07))),
            exponentiate=T, conf.int = T
),
broom::tidy(glm(outcome~factor(final_05), data=cbind.data.frame(mydata_preds3,final_07=posterior_glca_07_final$final_07,final_05=posterior_glca_05_final$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0))),
            exponentiate=T, conf.int = T
),
broom::tidy(glm(outcome~factor(final_07), data=cbind.data.frame(mydata_preds3,final_07=posterior_polca_07_final$final_07,final_05=posterior_polca_05_final$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0)) %>% dplyr::filter(!is.na(final_07))),
            exponentiate=T, conf.int = T
),
broom::tidy(glm(outcome~factor(final_05), data=cbind.data.frame(mydata_preds3,final_07=posterior_polca_07_final$final_07,final_05=posterior_polca_05_final$final_05)%>% dplyr::mutate(outcome=ifelse(outcome==1,1,0))),
            exponentiate=T, conf.int = T
)
) %>% 
